...
public:
  ...
  void printName()
  {
    ...
    color1(2);
    gotoxy(55, 1);
    cout << ch1 << ch1 << "Muhammad Hashim" << ch1 << ch1 << "2022-CS-18" << ch1 << ch1 << "DSA BY Sir Faraz" << ch1 << ch1 << endl;
    color1(7);
  }
  void run()
  {
      ...
      else if (usercommand == "copy")
      {
        ...
        if (file.length() > 0 && dotpostion != -1)
        {
          ...
          if (CurrentFolder->CheckFile(filename, fileExtension))
          {
            ...
            cin>>newdirector;
            Folder* CopiedDirectory=CurrentFolder->CheckDirectory(newdirector,Root,CurrentFolder);
